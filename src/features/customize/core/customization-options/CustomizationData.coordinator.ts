 ...
 *
 * Previews are generated at the category level: entering a category kicks off
 * preview generation for every pending option in display order (first to last),
 * each applied on top of the category's base image. Requests run in parallel;
 * only the trigger order is sequential. Selecting an option then commits its
 * already-generated preview, and leaving the category checkpoints the
 * combination.
 */
export class CustomizationDataCoordinator {
  ...
  /**
   * Combination strings with a preview request currently in flight. Prevents a
   * concurrent re-entry (e.g. React StrictMode double-invoking the mount effect,
   * or a rapid category switch) from firing a duplicate request for the same
   * option before the first one resolves and populates {@link previewImageCache}.
   */
  ...
  /**
   * Entering a category generates the preview for every pending option in
   * parallel, triggered in display order (first to last), each applied on top
   * of the category's base image (the current build without this category's own
   * selection). Previews already in the image cache are reused instantly; the
   * rest are generated and cached.
   */
  public async enterCategory(category: CustomizationCategory): Promise<void> {
    ...
    // Trigger in display order (first to last), but run all generations in
    // parallel so previews finish as soon as the backend allows.
    const generations: Promise<void>[] = [];
    for (const item of pending) {
      // The user may have moved to another category/base while we were
      // seeding; stop starting new work so we don't produce stale previews.
      ...
      // Skip options already being generated by a concurrent enterCategory call
      // so we don't fire duplicate requests for the same preview.
      if (this.inFlightPreviews.has(combinationString)) {
        continue;
      }
      this.inFlightPreviews.add(combinationString);
      generations.push(
        this.generateOptionPreview(
          category,
          baseString,
          baseSelections,
          baseImage,
  ...
  private async generateOptionPreview(
    category: CustomizationCategory,
    baseString: string,
    baseSelections: CombinationSelections,
    baseImage: string,
    ...
    try {
      ...
      // Ignore stale results if the category base changed while generating.
      if (this.categoryPreviewBase.get(category) !== baseString) {
        return;
      }
      ...
      this.inFlightPreviews.delete(combinationString);
    }
  }
